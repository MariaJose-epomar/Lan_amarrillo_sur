...
DATA_SECTION
 ...
 //Prepara datos para estimar mortalidad cuando Desembarque=0
 ...
 //!!cout << "ft_count "<< ft_count << endl;exit(1);
 
 !! ad_comm::change_datafile_name("lansur_es.ctl");
...
// parametros reclutamientos, desvíos R, No y mortalidades)
 ...
 init_bounded_vector log_Fm(1,ft_count,-20,-0.2,phs_F); // // log  mortalidad por pesca por flota machos F LIMITADA EN 0.8187 !!!!!!
 init_bounded_vector log_Fh(1,ft_count,-20,-0.2,phs_F); // log  mortalidad por pesca por flota
...
FUNCTION Eval_mortalidades
 ...
 for(i=1;i<=nyears;i++){
	 ftmp=0.00001;
	 if(Desemb(i)>0){
		 ftmp = mfexp(log_Fm(ki++));
	 }
	 ft(i)=ftmp;
 }
 Fm=elem_prod(Sel_flom,outer_prod(ft,Unos_edad));
 ...
 for(i=1;i<=nyears;i++){
	 ftmp=0.00001;
	 if(Desemb(i)>0){
		 ftmp = mfexp(log_Fh(ki++));
	 }
	 ft(i)=ftmp;
 }
 fyrh = ft; 
 Fh=elem_prod(Sel_floh,outer_prod(ft,Unos_edad));
...
FUNCTION Eval_abundancia
 ...
 No(1)=exp(log_Ro-0.5*square(cvar(1)))*(1-exp(log_propmR)); // Reclutamiento inexplotado hembras abundancia
 ...
 // Stock-recluta
 alfa=4*h*exp(log_Ro-0.5*square(cvar(1)))/(5*h-1);//reclutamiento total
 beta=(1-h)*SSBo/(5*h-1);//  


// genero una estructura inicial en equilibrio para el primer a�o
 Neqh(1)=mfexp(log_Ro-0.5*square(cvar(1)))*(1-exp(log_propmR));//hembras
 for (j=2;j<=nedades;j++)
 {
...
//   Neqh(nedades)+=Neqh(nedades)*exp(-1.*Zh(1,nedades)); // MODIFICAR POR LA OTRA FORMA
 ...
 Neqm(1)=mfexp(log_Ro-0.5*square(cvar(1)))*exp(log_propmR);//machos 
 for (j=2;j<=nedades;j++)
 {
...
// Abundancia inicial
 Nm(1)=elem_prod(Neqm,exp(log_dev_Nom-0.5*square(cvar(2))));
 Nh(1)=elem_prod(Neqh,exp(log_dev_Noh-0.5*square(cvar(2))));
 
 BD(1)=sum(elem_prod(elem_prod(Nh(1),exp(-dt(1)*Zh(1)))*Prob_talla_h,elem_prod(msex,Wmed(2))));

 Rpred(1)=mfexp(log_Ro-0.5*square(cvar(1)));//

// se estima la sobrevivencia por edad(a+1) y año(t+1)
 for (i=1;i<nyears;i++)
 {
     Rpred(i+1)=mfexp(log_Ro-0.5*square(cvar(1)));
	 // Reclutamiento estimado por un modelo B&H hembras
     if(i>=vec_ages(1)){
     Rpred(i+1)=alfa*BD(i-vec_ages(1)+1)/(beta+BD(i-vec_ages(1)+1))*exp(-0.5*square(cvar(1)));
 	}
     Nm(i+1,1)=Rpred(i+1)*mfexp(log_dev_Ro(i))*exp(log_propmR);  // Reclutas machos   
     Nh(i+1,1)=Rpred(i+1)*mfexp(log_dev_Ro(i))*(1-exp(log_propmR));// Reclutas hembras
...
//   Neqh(nedades)+=Neqh(nedades)*exp(-1.*Zpbr(nedades)); // MODIFICAR POR LA OTRA FORMA
 ...
 Frpr = fyrh/mfexp(log_Fref(3));
 
 //cout << "Frpr "<< pred_CPUE << endl;exit(1);
...
FUNCTION Eval_funcion_objetivo
 ...
 dvariable temp; 
 suma4=0; suma5=0; suma6=0; suma7=0; penalty=0; temp=0;
 //double sig_c=cv_index(1);
 ...
 for(int i=1;i<=nyears;i++){
	 if(Desemb(i)!=0){
	 temp+=square(log(Desemb(i)/pred_Desemb(i)))/square(cv_index(1,i));}
 }
...
// lognormal Ninicial y Reclutas
 ...
 for(int i=1;i<=nyears;i++){
	epsilon(i)=log(Nh(i,1)+Nm(i,1))-log(Rpred(i));}
	ssq = sum(square(epsilon));
//penalty(2)=0.5*nanos*log(2*M_PI)+nanos*log(cv_Ro)+ssq/(2*square(cv_Ro));
 penalty+=1/(2*square(cvar(1)))*ssq;
...
// se considera el Fpbr de hembras como el representativo factor limitante
 ...
 { // son # PBR only!
	...
	for (int i=1;i<=nyear_proy;i++)
	{
		...
		if(j==3){ //HCR 0-40
		    if(BDp/Bobj < 1){Fref=Fobj*(BDp/Bobj);
		    if(BDp/Bobj < 0.5){Fref=0.00001;}
		    }
		    else{Fref=Fobj;}
		}
		if(j==4){ //HCR 20-40
		    if(BDp/Bobj<1){Fref=Fobj*(BDp/Bobj-0.5)/0.5;
		    if(BDp/Bobj < 0.5){Fref=0.00001;}
		    }
		     else{Fref=Fobj;}
		}
		Fpbrh=Fref*Sel_floh(nyears);
		Fpbrm=Fref*Sel_flom(nyears);
		Zpbrh=Fpbrh+Mh;
		Zpbrm=Fpbrm+Mm;
		// año siguiente
		...
		Nph(nedades)+=Npplus;
		//Rp=alfa*BDp/(beta+BDp)*(1-exp(log_propmR))*exp(-0.5*square(cvar(1)));
		Rp=pRec*exp(log_Ro-0.5*square(cvar(1)));
		Nph(1)=Rp*(1-exp(log_propmR));

		Npplus=Npm(nedades)*Spm(nedades);
		Npm(2,nedades)=++elem_prod(Npm(1,nedades-1),Spm(1,nedades-1));
		Npm(nedades)+=Npplus;
		Npm(1)=Rp*exp(log_propmR);
 ...
 //CBA=YTP;// es para el año proyectado
 ...
 //TEMPORAL PARA EVALUAR LA RELACION STOCK_RECLUTA
 //ofstream ssbout("Bdesovante.txt");
 //ssbout << SSBp << endl; ssbout.close();
 //ofstream rtout("Reclutamiento.txt");
 //rtout << Rtp << endl; rtout.close();
