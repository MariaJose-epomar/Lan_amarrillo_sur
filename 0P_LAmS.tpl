GLOBALS_SECTION
 ...
 ofstream rep5("05FMort_hembras_op.txt",ios::app);
 ofstream rep6("06FMort_machos_op.txt",ios::app);
 //Nombre de archivos para las variables de estado del estimador
  ofstream rep7("02BiomasaTotal_est.txt",ios::app);
  ofstream rep8("03Desovante_est.txt",ios::app);
  ofstream rep9("04Reclutamiento_est.txt",ios::app);
  ofstream rep10("05FMort_hembras_est.txt",ios::app);
  ofstream rep11("06FMort_machos_est.txt",ios::app);
 //Nombre de archivos para fase histórica del simulador agregado por LCubillos
  ofstream rep12("07Fmort_hembras_hist.txt",ios::app);
  ofstream rep13("07Fmort_machos_hist.txt",ios::app);
  ofstream rep14("08Desovante_hist.txt",ios::app);
  ofstream rep15("09BiomTotal_hist.txt",ios::app);
  ofstream rep16("10Reclutas_hist.txt",ios::app); 
...
DATA_SECTION
 ...
 init_int nedades
 init_int ntallas
 init_matrix data(1,nyears,1,13);     //Years/desem/cpue/Bcru/ph/cv_desem/cv_CPUE/cv_Cru/cv_ph/n_mf/n_hf/n_mc //n_hc
 init_3darray Catsize(1,4,1,nyears,1,ntallas);

 !! ad_comm::change_datafile_name("lansur_op.ctl");
...
//Defino las variables de estado 
 ...
 //MODELO OPERATIVO: DIMENSIONAMIENTO DE NUMEROS, VECTORES Y MATRICES
 number Npplus
 number Bph;
 number Bpm;
 number BTpp;
 ...
 vector NVcru_m_fut(1,ntallas);
 vector NVcru_h_fut(1,ntallas);
 vector NVflo_m_fut(1,ntallas);
 vector NVflo_h_fut(1,ntallas);
 vector BMflo_fut(nyears+1,nyears+nyear_proy);
 vector BMcru_fut(nyears+1,nyears+nyear_proy);
 vector bvuln_fut(nyears+1,nyears+nyear_proy)
 vector sim_cpue(nyears+1,nyears+nyear_proy);
 vector sim_bcru(nyears+1,nyears+nyear_proy);
 vector cpue_epsilon_fut(nyears+1,nyears+nyear_proy);
 vector rec_epsilon_fut(nyears+1,nyears+nyear_proy);
 vector bcru_epsilon_fut(nyears+1,nyears+nyear_proy);
 vector prop_h_epsilon_fut(nyears+1,nyears+nyear_proy);
 vector prop_h_pred(nyears+1,nyears+nyear_proy);
 vector prop_h_fut(nyears+1,nyears+nyear_proy);
 
 vector yrs_fut(nyears+1,nyears+nyear_proy);
 
 matrix C_fut(1,npbr,nyears+1,nyears+nyear_proy);
 matrix Fm_fut(nyears+1,nyears+nyear_proy,1,nedades);
 matrix Fh_fut(nyears+1,nyears+nyear_proy,1,nedades);
 matrix fut_Ctot_am(nyears+1,nyears+nyear_proy,1,nedades);
 matrix fut_Ctot_ah(nyears+1,nyears+nyear_proy,1,nedades);
 matrix fut_Ctotm(nyears+1,nyears+nyear_proy,1,ntallas);
 matrix fut_Ctoth(nyears+1,nyears+nyear_proy,1,ntallas);
 matrix fut_pflo_m(nyears+1,nyears+nyear_proy,1,ntallas);
 matrix fut_pflo_h(nyears+1,nyears+nyear_proy,1,ntallas);
 matrix fut_pcru_m(nyears+1,nyears+nyear_proy,1,ntallas);
 matrix fut_pcru_h(nyears+1,nyears+nyear_proy,1,ntallas);
 
 //Para guardar los indices de estatus del estimador y evaluar desempeño del estimador
 matrix keep_Btot(1,npbr,nyears+1,nyears+nyear_proy);
 matrix keep_SSBt(1,npbr,nyears+1,nyears+nyear_proy);
 matrix keep_Rt(1,npbr,nyears+1,nyears+nyear_proy);
 matrix keep_Fm(1,npbr,nyears+1,nyears+nyear_proy);
 matrix keep_Fh(1,npbr,nyears+1,nyears+nyear_proy);
 ...
 matrix des_Fm(1,npbr,nyears+1,nyears+nyear_proy);
 matrix des_Fh(1,npbr,nyears+1,nyears+nyear_proy);
...
FUNCTION OP_Model
 ...
 dvector epsilon_rec_fut(nyears+1,nyears + nyear_proy);
 dvector epsilon_cpue_fut(nyears+1,nyears + nyear_proy);
 dvector epsilon_bcru_fut(nyears+1,nyears + nyear_proy);
 dvector epsilon_prop_h_fut(nyears+1,nyears+ nyear_proy);
 int upk;
 int k;
 dvariable numyear=1979;
 dvector yrs(nyears+1,nyears + nyear_proy);
 simname="LamS.dat"; //nombre de los datos del estimador
 dvector CatchNow(1,npbr);
 dvector  eval_now(1,5);
 dvariable grad_tmp;

 epsilon_rec_fut.fill_randn(rng);
 epsilon_cpue_fut.fill_randn(rng);
 epsilon_bcru_fut.fill_randn(rng);
 epsilon_prop_h_fut.fill_randn(rng);
 int rv;
 
 for(int l=1;l<=npbr;l++){
	 rv=system("cp LAmS_bkp.dat LamS.dat");
	 rv=system("./LamS -nox -nohess");
	Nph=Nh(nyears);
	Npm=Nm(nyears);
    Sph=Sh(nyears);
    Spm=Sm(nyears);
	...
	for(int i=nyears+1;i<=nyears + nyear_proy;i++)
	{
		//cvar(1)=sigmar_fut tratar de que sea mas bajo aprox. 0.3 o 0.4
		//TODO: considerar reclutamientos autocorrelacionados
		
		rec_epsilon_fut(i)=exp(epsilon_rec_fut(i)* cvar(1));
		cpue_epsilon_fut(i)=exp(epsilon_cpue_fut(i)* 0.15);
		bcru_epsilon_fut(i)=exp(epsilon_bcru_fut(i)* 0.3);
		prop_h_epsilon_fut(i)=exp(epsilon_prop_h_fut(i)* 0.05);
	    ...
	    Nph(nedades)+=Npplus;
		if(ErrorProc==1){
	    Nph(1)=alfa*BDp/(beta+BDp); 
	    }
		if(ErrorProc==2){
	    Nph(1)=alfa*BDp/(beta+BDp)*exp(rec_epsilon_fut(i)); 
	    }		
	    Npplus=Npm(nedades)*Spm(nedades);
		Npm(2,nedades)=++elem_prod(Npm(1,nedades-1),Spm(1,nedades-1));
	    Npm(nedades)+=Npplus;
	    Npm(1)=Nph(1)*exp(log_propmR)/(1-exp(log_propmR));	

		//Seccion para estimar la F que genera la CBA leida desde el estimador
	    NVflo_m_fut = (elem_prod(Npm,Sel_flom(nyears)))*Prob_talla_m; //NVflo_m_fut          es un vector dim(1:ntallas) inicial sin efectos de exp(-dt(2)*Zph) ya            que no conocemos F (el cual se estimará con Newton-Raphson a       continuación)
	    NVflo_h_fut = (elem_prod(Nph,Sel_floh(nyears)))*Prob_talla_h;
		BMflo_m_fut = NVflo_m_fut * Wmed(1); //Biomasa vulnerable machos 
		BMflo_h_fut = NVflo_h_fut * Wmed(2); //Biomasa vulnerable hembras
		
		// Proporcion de hembras observado en las captura
		//prop_h_fut(i) = prop_h_pred(nyears);
        prop_h_fut(i) = 0.3;
						
		if(C_fut(l,i)!=0){
			//Estima la mortalidad asumiendo F por sexo desde captura total sobre biomasa vulnerable parcial por sexo: TODO: verificar supuesto
			//machos
			bvuln_fut(i)=BMflo_m_fut;
			M_tmp = Mm;
			sel = Sel_flom(nyears);
			dvariable ffpen=0.0;
			dvariable SK = posfun((bvuln_fut(i) - prop_h_fut(i)*C_fut(l,i))/bvuln_fut(i),0.05,ffpen);
			Kobs_tot_catch= bvuln_fut(i)-SK*bvuln_fut(i);
			do_Newton_Raphson_for_mortality(i);
			Fm_fut(i)=F_tmp(i);
			//Hembras
			bvuln_fut(i)=BMflo_h_fut;
			M_tmp = Mh;
			sel = Sel_floh(nyears);
			ffpen=0.0;
			SK = posfun((bvuln_fut(i) - (1-prop_h_fut(i))*C_fut(l,i))/bvuln_fut(i),0.05,ffpen);
			Kobs_tot_catch= bvuln_fut(i)-SK*bvuln_fut(i);
			do_Newton_Raphson_for_mortality(i);
			Fh_fut(i)=F_tmp(i);
		}
				...
				Sph   = exp(-1*Zpbrh);
				//Biomasa vulnerable
				NVflo_m_fut = elem_prod(elem_prod(Npm,mfexp(-dt(2)*Zpbrm)),Sel_flom(nyears))*Prob_talla_m;
				NVflo_h_fut = elem_prod(elem_prod(Nph,mfexp(-dt(2)*Zpbrh)),Sel_floh(nyears))*Prob_talla_h;
				BMflo_m_fut = NVflo_m_fut * Wmed(1); //Biomasa vulnerable machos 
				BMflo_h_fut = NVflo_h_fut * Wmed(2); //Biomasa vulnerable hembras
				BMflo_fut(i) = BMflo_m_fut + BMflo_h_fut; //Biomasa vulnerable total
			
				//Crucero
				NVcru_m_fut = elem_prod(elem_prod(Npm,mfexp(-dt(3)*(Zpbrm))),Sel_crum(nyears))*Prob_talla_m;
				NVcru_h_fut = elem_prod(elem_prod(Nph,mfexp(-dt(3)*(Zpbrh))),Sel_cruh(nyears))*Prob_talla_h;
				BMcru_fut(i) = NVcru_m_fut*Wmed(1) + NVcru_h_fut*Wmed(2);
				//Biomasa total
			    Bph=sum(elem_prod(Nph*Prob_talla_h,Wmed(2)));
			    Bpm=sum(elem_prod(Npm*Prob_talla_m,Wmed(1)));
				BTpp = Bph + Bpm; //Biomasa total 
			    NMDp=elem_prod(Nph,mfexp(-dt(1)*Zpbrh))*Prob_talla_h; //Numero desovante
			    BDp=sum(elem_prod(elem_prod(NMDp,msex),Wmed(2))) ; //biomasa desovante
				
				//======== SIMULA LOS DATOS PARA ACTUALIZAR LA EVALUACION CON EL ESTIMADOR =====
				//CPUE
				if(ErrorObs==1){
				sim_cpue(i) = exp(log_qflo(nbloq_qflo))*BMflo_fut(i); 
				//Biomasa crucero
				sim_bcru(i) = exp(log_qcru(nbloq_qcru))*BMcru_fut(i); 
			    }
				if(ErrorObs==2){
			    sim_cpue(i) = exp(log_qflo(nbloq_qflo))*BMflo_fut(i)*exp(cpue_epsilon_fut(i));
				//Biomasa crucero
				sim_bcru(i) = exp(log_qcru(nbloq_qcru))*BMcru_fut(i)*exp(bcru_epsilon_fut(i));
				}
				...
				for(int j=1;j<=nedades;j++){//captura por edad
					fut_Ctot_am(i,j) = (Fm_fut(i,j)/Zpbrm(j))*Npm(j)*(1.-Spm(j));
				}
				fut_Ctotm(i) = fut_Ctot_am(i)*Prob_talla_m;
				for(int j =1; j<=ntallas;j++){
					fut_pflo_m(i,j)=fut_Ctotm(i,j)/sum(fut_Ctotm(i)+1e-10);
					fut_pcru_m(i,j)= NVcru_m_fut(j)/sum(NVcru_m_fut+1e-10);		
				}
				//hembras
				for(int j=1;j<=nedades;j++){
					fut_Ctot_ah(i,j) = (Fh_fut(i,j)/Zpbrh(j))*Nph(j)*(1.-Sph(j));
				}
				fut_Ctoth(i)   = fut_Ctot_ah(i)*Prob_talla_h;
				for(int j=1; j<=ntallas;j++){
					fut_pflo_h(i,j) = fut_Ctoth(i,j)/sum(fut_Ctoth(i)+1e-10);
					fut_pcru_h(i,j) = NVcru_h_fut(j)/sum(NVcru_h_fut+1e-10);
				}
				    
			    //prop_h_pred = elem_div(rowsum(pred_Ctoth),rowsum(pred_Ctoth+pred_Ctotm+1e-10));
				//prop_h_fut(i) = prop_h_pred(nyears);
	    	    prop_h_fut(i) = 0.3;
				...
				//numyear = yrs_fut(i)+i-1;
				//cout << "yrs_fut = " << yrs_fut << endl;
				...
				for(k=nyears+1;k<=upk;k++){
					simdata << " "<< yrs_fut(k) << "  " << C_fut(l,k) << "  " << sim_cpue(k) << "  " << sim_bcru(k) << "  " << prop_h_fut(k) << "  " << 0.1 << "  " << 0.15 << "  " << 0.3 << "  " << 0.05 << "  "<< 115 << "  " << 70 << "  " << 211 << "  " << 60 << "  "<< endl;
				}
				simdata << "#Estructura de tallas historica" << endl;
				simdata << "# Machos flota " << endl;
				simdata << Catsize(1) << endl; //TODO: revisar
				simdata << "# Machos flota simulados " << endl;
				for(k=nyears+1;k<=upk;k++){
					simdata << fut_pflo_m(k) << endl;
				}
				simdata << "# Hembras flota historica " << endl;
				simdata << Catsize(2) << endl;
				simdata << "# Hembras flota simulados " << endl;
				for(k=nyears+1;k<=upk;k++){
					simdata << fut_pflo_h(k) << endl;
				}
				simdata << "# Machos crucero historico " << endl;
				simdata <<  Catsize(3) << endl;
				simdata << "# Machos crucero simulados " << endl;
				for(k=nyears+1;k<=upk;k++){
					simdata  <<fut_pcru_m(k) << endl;
				}
				simdata << "# Hembras crucero historico" << endl;
				simdata <<  Catsize(4) <<endl;
				simdata << "# hembras crucero simulados " << endl;
				for(k=nyears+1;k<=upk;k++){
				...
				keep_Fh(l,i) =eval_now(4); //Mortalidad por pesca machos
				keep_Fm(l,i) =eval_now(5); //Mortalidad por pesca hembras
				//Guarda los indicadores del simulador que debío estimar el estimador (valga la redundancia)
				des_Btot(l,i)=BTpp; //Biomasa total
				des_SSBt(l,i)=BDp; //Biomasa desovante
				des_Rt(l,i) = Nph(1); //Reclutamiento hembras
				des_Fh(l,i) = max(Fh_fut(i)); //Mortalidad por pesca machos 
				des_Fm(l,i) = max(Fm_fut(i)); //Mortalidad por pesca hembras
				//======LEE GRADIENTE ===
				ifstream lee_grad("grad_final.dat");
				lee_grad >> grad_tmp;
				lee_grad.close();
				if(grad_tmp>0.001){check_convergence(l,i)=0;}else{check_convergence(l,i)=1;}
				//cout << "des_Fm" << des_Fm << endl; exit(1);
				//cout << "HASTA AQUI TODO BIEN" << endl; exit(1);
				} //cierra ciclo de proyeccion
	
		 } //cierra ciclo del numero de HCR
		 //======== REPORTA SIMLACIONES Y ESTIMACIONES
		 //Capturas efectivas
		 //TODO: trabajar el error de implementacion de la medida 
		 rep1 << C_fut << endl;
		 ...
		 rep12 << mfexp(log_Fh) << endl; //mortalidad pesca hembras
         rep13 << mfexp(log_Fm) << endl; //mortalidad pesca machos
		 rep14 << BD << endl; //desovante
		 rep15 << BT << endl; //biomasa total
		 rep16 << Restim << endl; //Reclutas hembras	 
...
REPORT_SECTION
 ...
 rep(Rpred);
 rep(column(Nh,1));
 rep(rowsum(Fm)/nedades);
 rep(rowsum(Fh)/nedades);
 rep(prop_h);
 rep(prop_hpred);
 rep(vec_tallas*trans(pobs_flom));
 rep(vec_tallas*trans(ppred_flom));
 rep(vec_tallas*trans(pobs_floh));
 rep(vec_tallas*trans(ppred_floh));
 rep(vec_tallas*trans(pobs_crum));
 rep(vec_tallas*trans(ppred_crum));
 rep(vec_tallas*trans(pobs_cruh));
 rep(vec_tallas*trans(ppred_cruh));
 rep(Sel_flom);
 rep(Sel_floh);
 rep(Sel_crum);
 rep(Sel_cruh);
 ...
 rep(RPRlp);
 rep(exp(log_Loh));
 rep(exp(log_cv_edadh));
 rep(exp(log_Lom));
 rep(exp(log_cv_edadm));
 rep(Mm);
 rep(Mh);
 rep(mu_edadm);
 rep(mu_edadh);
