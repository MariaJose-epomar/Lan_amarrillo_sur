...
FUNCTION OP_Model
 ...
 for(int l=1;l<=npbr;l++){
	...
	for(int i=nyears+1;i<=nyears + nyear_proy;i++)
	{
	    ...
	    Nph(1)=alfa*BDp/(beta+BDp)*exp(rec_epsilon_fut(i)); //cambia a BDp 
		...
		//cout << "HASTA AQUI..prop" << NVflo_h_fut<< endl;exit(1);
		...
		//cout << "HASTA AQUI..prop" << BMflo_h_fut << endl;exit(1);
		//cout << "HASTA AQUI..prop =" << prop_h_pred << endl;exit(1);

		// Proporcion de hembras observado en las captura
		// Por ahora asumo 0.5
		prop_h_fut(i) = prop_hpred(nyears); //hacer variable para genetrar errror de proceso
		//prop_h_fut(i) = 0.5;
		//cout << "HASTA AQUI..prop" << prop_h_fut << endl;exit(1);
		...
		if(C_fut(l,i)!=0){
			...
			do_Newton_Raphson_for_mortality(i);
			Fh_fut(i)=F_tmp(i);
			//cout << "HASTA AQUI.." << Fh_fut << endl;exit(1);
		}
		else{
					Fm_fut(i) = 0;
					Fh_fut(i) = 0;
		 ...
		 rep10 << keep_Fh << endl;
		 rep11 << keep_Fm << endl;
		 //rep12 << BD << endl; incorporar parte historica y hay que agregarlo arriba
		 //rep13 << mfexp(log_Fh)<<endl;
